...
DALRRD has formally requested poisoning intelligence to inform registration, labelling, and inspectorate priorities. Output: a quarterly NICD brief by suspected product / geography / season, plus an ad-hoc case-spike alert channel. **Closes the regulator-feedback gap and gives DALRRD the evidence base for product-specific action (the explicit prerequisite for the [Terbufos brief](terbufos_policy.qmd) recommendation).** Annual cost: **~R40,000** (NICD analyst + DALRRD liaison), 12 weeks.

### Option C4 — Auto-feedback to notifying clinician

NMC sends the notifying clinician a quarterly case-status summary: outcome of EHP investigation, regulatory action taken, whether their case contributed to a wider investigation. Removes the "I notify into a black hole" disincentive. Annual cost: **~R20,000** (templating + email integration).

### Option C5 — Joint KPI dashboard

A small public dashboard tracking: median time from notification to EHP investigation; % of notifications with a recorded outcome; % of cases with documented DALRRD action; % of notifying clinicians who received feedback. Co-owned by NICD, DALRRD, and provincial DoHs. Annual cost: **~R30,000**.
