...
**How to interpret.** Items 1 and 5 are direct cash outlays (cloud hosting + licence). Items 2, 3, 4 and 6 are time-allocated personnel cost at standard public-sector CTE rates — the same anchors used in §7 Tab 1. The NHLS medical scientist and data-engineer CTE rows already appear in the status-quo build-up; the Option 1 marginal allocation is additive to the existing pesticide share of those posts.

**Distribution.** gamma(shape = 25, scale = 2,200) → mean R55,000, SD ≈ R11,000 (95% CI ≈ R36k–R78k).

**Sensitivity.** Dominant driver: middleware hosting tier (upgrading t3.small → t3.medium adds ~R12,000/yr). Doubling the NHLS IT maintenance FTE share adds ~R15,000/yr. Neither moves Option 1 above Option 2 in operating cost (R82,000/yr).
...
**P_detect impact: by design, none directly.** Option 4 does **not** increase the share of true cases entering surveillance — that is the role of Options 1, 2 and 3. Its value lies in **converting detected cases into actionable investigations**: it is the data-quality bridge that allows the coordination response to fire on the right cases, with the right product information, at the right site. The structured agent-attribution field added in Option 2 is the *what*; the CIF + agricultural extension is the *where, how, and who*. Together they make NMC the cross-sectoral hand-off point from health-sector detection into the DALRRD + DoH source-removal pipeline.

**Why this belongs in the surveillance brief, not the coordination brief.** The form lives inside NMC; its build, governance and KPIs are NDoH-owned surveillance infrastructure. The [Coordination brief](coordination_policy.qmd) *consumes* the structured CIF + extension outputs to trigger source removal and inter-departmental escalation — it does not own the form itself. Option 4 is therefore the natural seam between the two briefs: it modestly upgrades NMC to **pave the way for coordination**, without requiring any new legislation (the CIF already exists in the 2023 guideline; Section 90 of the National Health Act and the NMC regulations already authorise the data flows).
...
**Option 5 has a different purpose from Options 1 and 2** and is *not* a detection-coverage uplift. Options 1 and 2 increase the share of true cases that enter the surveillance system. **Option 5 identifies the proportion of chemical agents being found in severely poisoned living patients at sentinel sites** — the missing evidence base for product-specific regulation (see [Terbufos brief](terbufos_policy.qmd)). Without Option 5, ~10,000 survivors/year are recorded with agent "unknown / suspected" and targeted product restriction has no quantitative footing. Some infromation is being captured in the Injury and MOrtality Survey by Megan Prinsloo et al (*** NOTE FOR THE AUTHOER ***)
...
**Why Option 5 is funded from research streams, not the surveillance operating budget.** Option 5 is a **research investment** — it generates the agent-attribution evidence base for targeted regulatory action. It does not increase P_detect (the share of true cases entering the surveillance system) and therefore does not appear in the Option 1, Option 2 or MVP operating-cost parameters. Its value is quantified in the [Terbufos brief](terbufos_policy.qmd) where regulatory restriction converts agent-attribution evidence into deaths averted.
...
| Option 1 — BChE auto-notify *(first build; CSV: `c_S2_*`)* | `r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")))` | `r zar(as.numeric(pick("c_S2_BChE_autonotify_op")))` | `r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")) + as.numeric(pick("c_S2_BChE_autonotify_op")))` | $\frac{1,013 + 1,779}{20,960} = 0.133$ | `r sprintf("+%.1f pp", (as.numeric(pick("d_S2_BChE_autonotify")) - as.numeric(pick("d_status_quo"))) * 100)` | `r paste0(zar(round(as.numeric(pick("c_S2_BChE_autonotify_op")) / ((as.numeric(pick("d_S2_BChE_autonotify")) - as.numeric(pick("d_status_quo"))) * 100))), " / pp")` | 2–4 wks | [`d_S2_BChE_autonotify`](../parameters_hub.qmd#tbl-detection) |
| Option 2 — PIH dashboard + MVD (+AfriTox agent name) *(second build; CSV: `c_S1_*`)* | `r zar(as.numeric(pick("c_S1_PIH_MVD_setup")))` | `r zar(as.numeric(pick("c_S1_PIH_MVD_op")))` | `r zar(as.numeric(pick("c_S1_PIH_MVD_setup")) + as.numeric(pick("c_S1_PIH_MVD_op")))` | $\frac{1,013 + 1,158}{20,960} = 0.104$ | `r sprintf("+%.1f pp", (as.numeric(pick("d_S1_PIH_MVD")) - as.numeric(pick("d_status_quo"))) * 100)` | `r paste0(zar(round(as.numeric(pick("c_S1_PIH_MVD_op")) / ((as.numeric(pick("d_S1_PIH_MVD")) - as.numeric(pick("d_status_quo"))) * 100))), " / pp")` | 1–2 wks dash / 3–9 mo MVD | [`d_S1_PIH_MVD`](../parameters_hub.qmd#tbl-detection) |
...
: **Intermediate-outcome costing analysis — surveillance options, base-case.** Costs are 2024–25 ZAR; P_detect is the per-case detection probability anchored to the StatsSA MACOD 2023 mid-burden of `r scales::comma(as.numeric(pick("n_true_burden_from_statssa_mid")))` true cases/year. †**R / pp** = annual operating cost ÷ percentage-point gain in P_detect over status quo (e.g. R9,716 / pp means each additional 1% of cases detected costs R9,716/yr to sustain). Options 1 and 2 expand coverage within the formal healthcare system; **Option 5 is a parallel agent-attribution investment**, not a coverage uplift — see option text above. **Option 4 is a data-quality / investigation-form upgrade** that converts detected cases into actionable source-removal hand-offs to the [Coordination brief](coordination_policy.qmd); it does not raise P_detect by design. **Option 3 is a Phase 2 community layer**, commissioned conditional on MVP pilot data confirming a structural rural/informal gap (trigger: ≥40% of severe cases in low-facility-access districts after 6 months of MVP operation) **and** on PIH/NMC opening a non-clinician intake channel — without that pre-condition the CHW network has no destination for its reports and Option 3 yields no P_detect gain. {.striped .hover}
...
**No double-counting.** The Option 1 and Option 2 cost items sit on entirely separate operational pathways:

- **Option 2 items** (PIH cloud hosting, MVD ETL pipeline, AfriTox harmonisation, QA, licences/SMS) are driven by PIH call-record flows and the NICD/PIH data-dictionary agreement.
- **Option 1 items** (NHLS HL7 middleware, LIS IT maintenance, NHLS scientist QA, NICD alert triage) are driven by the NHLS LIS auto-notification rule.

The two NICD analyst FTE allocations sum to ~0.05 FTE total (Option 2: ~0.035 FTE; Option 1: ~0.015 FTE) — within the 0.2 FTE analyst post already costed in the §7 Tab 1 NMC baseline. **The MVP does not require a new NICD headcount.**

**Setup.** `c_surv_MVP_setup` = R240,000 (one-off) = `c_S1_PIH_MVD_setup` (R40,000) + `c_S2_BChE_autonotify_setup` (R200,000). S0 is already operational and its setup cost was separately incurred; it is not double-counted here. The Option 1 LIS HL7 reconfiguration (R200,000) dominates.
:::
...
| S0 — NHLS-NMC IT feed (now operational, in baseline) | `c_S0_NHLS_NMC_feed_setup` | `c_S0_NHLS_NMC_feed_op` |
| Status quo (NICD program) | — | `c_status_quo_program` |

**Parameter codes vs option numbers.** The CSV parameter names (`c_S1_*`, `c_S2_*`, `c_S1b_*`, `c_option6_*`) are stable identifiers used across the model, decision tree (§6) and other policy briefs — they pre-date this brief's narrative reorder and intentionally do not follow the Option 1–5 display sequence. Use the cross-walk in the "Reading guide" callout at the top of §5 to translate between the two.
...
| Option 2 — PIH MVD + dashboard | NMC + PIH | $(1{,}013 + 1{,}158) / 20{,}960 = 2{,}171 / 20{,}960$ | **0.104** | `d_S1_PIH_MVD` |
| Option 1 — BChE auto-notify | NMC + NHLS severe | $(1{,}013 + 1{,}779) / 20{,}960 = 2{,}792 / 20{,}960$ | **0.133** | `d_S2_BChE_autonotify` |
| **Surveillance MVP (Option 1 + Option 2)** | NMC + NHLS severe + PIH | $(1{,}013 + 1{,}779 + 1{,}158) / 20{,}960 = 3{,}950 / 20{,}960$ | **0.189** | `d_surveillance_mvp` |
...
The coordination side of the same system — how NICD, EHPs, DALRRD, and provincial response teams act on the improved signal — is covered in the [Coordination brief](coordination_policy.qmd). The product-specific regulatory action that Option 5 makes possible is the subject of the [Terbufos brief](terbufos_policy.qmd).
...
**AfriTox agent-name harmonisation.** Both PIH (TMS) and NMC (notification form) must use the same AfriTox product-name controlled vocabulary so that product-level attribution is consistent across streams. The harmonisation workshop (Option 2 MVD workshop, Months 1–3) should produce a shared AfriTox lookup table embedded in both systems. This is the lowest-cost, highest-value interoperability intervention in the MVD: it costs a single workshop and yields cross-stream agent attribution at scale.
:::
