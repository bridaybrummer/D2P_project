...
The 20,960 denominator applies a 12.5% CFR to the StatsSA MACOD death count (ICD-10 X40–X49) — but that CFR derives from NMC-*notified* severe cases (the sicker end of the distribution). The undetected majority are likely milder; a blended CFR of 3–5% implies 52,000–87,000 true cases and a status-quo P_detect of 1–2%. Relative option rankings (S1, S2, MVP†) are stable across this range — the percentage-point gain for each option is unchanged; only the absolute number of true cases detected would differ.

†MVP = Minimum Viable Package (S1 + S2 combined).
:::
...
Once the integrated streams are live, historical burden estimates should be re-calibrated against the expanded data. Reported case counts will rise as a larger proportion of the true burden is detected — this is a surveillance artefact, not a true increase in poisonings, and must be communicated clearly in any public reporting. 
...
### Option S2 — BChE auto-notification *(★ recommended — first build, capability exists)*

Configure the NHLS LIS to fire an automatic NICD notification when a BChE result shows severe inhibition (< 30 % of normal)*, within 24 hours of validation. **No new test, no new reagent contract** — only an IT configuration on top of the ~`r scales::comma(as.numeric(pick("n_nhls_total_annual")))` BChE tests/year already performed (`r scales::comma(as.numeric(pick("n_nhls_severe_annual")))` severe in 2023). Detection rises to ~`r sprintf("%.0f", as.numeric(pick("d_S2_BChE_autonotify")) * 100)` % of true cases (up from ~`r sprintf("%.0f", as.numeric(pick("d_status_quo")) * 100)` %) for a one-off setup of **`r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")))`**, then **`r zar(as.numeric(pick("c_S2_BChE_autonotify_op")))`/year** from Year 2 onward (Year 1 total: **`r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")) + as.numeric(pick("c_S2_BChE_autonotify_op")))`**) in 2–4 weeks. **This is the single highest-value, fastest, lowest-risk option on the menu — and is therefore the recommended first build.** It uses existing NHLS infrastructure, has a single clear owner (NHLS LIS), and delivers the largest single-option uplift in P_detect (+`r sprintf("%.1f pp", (as.numeric(pick("d_S2_BChE_autonotify")) - as.numeric(pick("d_status_quo"))) * 100)`).
...
### Option S1 — PIH live dashboard + Minimum Viable Dataset *(★ recommended — second build, completes the MVP)*
...
::: {.callout-important}
**Pre-condition — non-clinician reporting must be enabled.** S1b only delivers a coverage uplift if PIH (and the NMC CIF, via S4) accept structured reports from **non-medical personnel**: community health workers, environmental health practitioners, ward-based outreach teams, agricultural extension officers, teachers, traditional healers, and community members themselves. The MVP's S1 stream is built on the ~1,158 *clinician-initiated* PIH calls/year — that pathway by construction cannot see cases that never reach a clinician. Unless PIH opens a separate intake channel for non-clinician callers (with the Tier-1 CHW checklist as the structured form, and EHP call-back triage as the validation step), the CHW network has nowhere to land its reports and S1b reduces to training cost with no measurable P_detect gain. This is the single design choice that makes or breaks the community layer.
:::
...
### Option S4 — NMC case investigation form + agricultural extension *(bridge to coordination)*
...
1. **Operationalise the existing NDoH CIF inside NMC.** Embed the 2023 guideline's investigation fields as a structured second-stage form auto-triggered by any pesticide-poisoning notification — whether the notification originates from NMC, the S1 PIH MVD ingest, or the S2 BChE auto-notify rule. EHPs receive a workflow alert; CIF completion within 48 hours becomes a tracked NMC performance indicator (district-level KPI, reported quarterly).
2. **Add an agricultural-investigation extension module.** Additional structured fields layered onto the CIF for:
    ...
    - **AfriTox-coded agent name** (reciprocal with the S1 drop-down so PIH-reported and NMC-confirmed agent names are directly comparable);
...
**Cost.** ~R45,000 setup (form-template build into the NMC platform; one-day inter-departmental design workshop with NDoH EHP directorate, DALRRD Act-36 registrar, NICD, and PIH) and ~R35,000/yr operating (NICD analyst 0.04 FTE × R650k DPSA L10-11 for form-completion QA, drop-down maintenance and quarterly performance reporting to NDoH). **No new headcount** — same DPSA L10-11 analyst anchor used for S0/S1/S2; no new IT platform — the NMC notification system already supports structured secondary forms.

**P_detect impact: by design, none directly.** S4 does **not** increase the share of true cases entering surveillance — that is the role of S1, S1b and S2. Its value lies in **converting detected cases into actionable investigations**: it is the data-quality bridge that allows the coordination response to fire on the right cases, with the right product information, at the right site. The structured agent-attribution field added in S1 is the *what*; the CIF + agricultural extension is the *where, how, and who*. Together they make NMC the cross-sectoral hand-off point from health-sector detection into the DALRRD + DoH source-removal pipeline.

**Why this belongs in the surveillance brief, not the coordination brief.** The form lives inside NMC; its build, governance and KPIs are NDoH-owned surveillance infrastructure. The [Coordination brief](coordination_policy.qmd) *consumes* the structured CIF + extension outputs to trigger source removal and inter-departmental escalation — it does not own the form itself. S4 is therefore the natural seam between the two briefs: it modestly upgrades NMC to **pave the way for coordination**, without requiring any new legislation (the CIF already exists in the 2023 guideline; Section 90 of the National Health Act and the NMC regulations already authorise the data flows).

### Option S3 — Sentinel live-patient toxicology *(★ recommended, medium term — distinct purpose, research-funded)*

NHLS currently performs chemical toxicology (qualitative/quantitative pesticide assays) **post-mortem only**. There is no live-patient toxicology capability. Option S3 funds a sentinel tertiary site (e.g. CHB) to perform live-patient chemical toxicology and/or mass spectometry, scaling to 4–5 sites in phase 2–3. This will only capture ~ 323 severe poisoning cases. 
...
**S3 has a different purpose from S1 and S2** and is *not* a detection-coverage uplift. S1 and S2 increase the share of true cases that enter the surveillance system. **S3 identifies the proportion of chemical agents being found in severely poisoned living patients at sentinel sites** — the missing evidence base for product-specific regulation (see [Terbufos brief](terbufos_policy.qmd)). Without S3, ~10,000 survivors/year are recorded with agent "unknown / suspected" and targeted product restriction has no quantitative footing. Some infromation is being captured in the Injury and MOrtality Survey by Megan Prinsloo et al (*** NOTE FOR THE AUTHOER ***)
...
| S2: BChE auto-notify *(first build)* | `r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")))` | `r zar(as.numeric(pick("c_S2_BChE_autonotify_op")))` | `r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")) + as.numeric(pick("c_S2_BChE_autonotify_op")))` | $\frac{1,013 + 1,779}{20,960} = 0.133$ | `r sprintf("+%.1f pp", (as.numeric(pick("d_S2_BChE_autonotify")) - as.numeric(pick("d_status_quo"))) * 100)` | `r paste0(zar(round(as.numeric(pick("c_S2_BChE_autonotify_op")) / ((as.numeric(pick("d_S2_BChE_autonotify")) - as.numeric(pick("d_status_quo"))) * 100))), " / pp")` | 2–4 wks | [`d_S2_BChE_autonotify`](../parameters_hub.qmd#tbl-detection) |
| S1: PIH dashboard + MVD (+AfriTox agent name) *(second build)* | `r zar(as.numeric(pick("c_S1_PIH_MVD_setup")))` | `r zar(as.numeric(pick("c_S1_PIH_MVD_op")))` | `r zar(as.numeric(pick("c_S1_PIH_MVD_setup")) + as.numeric(pick("c_S1_PIH_MVD_op")))` | $\frac{1,013 + 1,158}{20,960} = 0.104$ | `r sprintf("+%.1f pp", (as.numeric(pick("d_S1_PIH_MVD")) - as.numeric(pick("d_status_quo"))) * 100)` | `r paste0(zar(round(as.numeric(pick("c_S1_PIH_MVD_op")) / ((as.numeric(pick("d_S1_PIH_MVD")) - as.numeric(pick("d_status_quo"))) * 100))), " / pp")` | 1–2 wks dash / 3–9 mo MVD | [`d_S1_PIH_MVD`](../parameters_hub.qmd#tbl-detection) |
| **S2 + S1 (Surveillance MVP)** | **`r zar(as.numeric(pick("c_surv_MVP_setup")))`** | **`r zar(as.numeric(pick("c_surv_MVP_op")))`** | **`r zar(as.numeric(pick("c_surv_MVP_setup")) + as.numeric(pick("c_surv_MVP_op")))`** | $\frac{1,013 + 1,779 + 1,158}{20,960} = 0.189$ | **`r sprintf("+%.1f pp", (as.numeric(pick("d_surveillance_mvp")) - as.numeric(pick("d_status_quo"))) * 100)`** | **`r paste0(zar(round(as.numeric(pick("c_surv_MVP_op")) / ((as.numeric(pick("d_surveillance_mvp")) - as.numeric(pick("d_status_quo"))) * 100))), " / pp")`** | **12 weeks** | [`d_surveillance_mvp`](../parameters_hub.qmd#tbl-detection) |
| S1b: Phase 2 community layer *(pilot: FS + Gauteng; national conditional on pilot; **requires PIH/NMC intake from non-clinicians**)* | `r sprintf("R%.0fM–R%.1fM", as.numeric(pick("c_S1b_setup_lo"))/1e6, as.numeric(pick("c_S1b_setup_hi"))/1e6)` | Pilot `r sprintf("R%.1fM/yr", as.numeric(pick("c_S1b_op_lo"))/1e6)`; national `r sprintf("R%.1fM/yr", as.numeric(pick("c_S1b_op_hi"))/1e6)` | `r sprintf("R%.1fM–R%.1fM", (as.numeric(pick("c_S1b_setup_lo")) + as.numeric(pick("c_S1b_op_lo")))/1e6, (as.numeric(pick("c_S1b_setup_hi")) + as.numeric(pick("c_S1b_op_hi")))/1e6)` | `r pick("d_S1b_event_community_lo")`–`r pick("d_S1b_event_community_hi")` (working: `r pick("d_S1b_event_community_mid")`) | `r sprintf("+%.0f to +%.0f pp", (as.numeric(pick("d_S1b_event_community_lo")) - as.numeric(pick("d_status_quo"))) * 100, (as.numeric(pick("d_S1b_event_community_hi")) - as.numeric(pick("d_status_quo"))) * 100)` | `r sprintf("%s–%s / pp", zar(round(as.numeric(pick("c_S1b_op_lo")) / ((as.numeric(pick("d_S1b_event_community_lo")) - as.numeric(pick("d_status_quo"))) * 100))), zar(round(as.numeric(pick("c_S1b_op_hi")) / ((as.numeric(pick("d_S1b_event_community_hi")) - as.numeric(pick("d_status_quo"))) * 100))))` | 12 mo pilot → national | [`d_S1b_event_community_mid`](../parameters_hub.qmd) |
| S4: NMC CIF + agricultural extension (bridge to coordination) | R45,000 | R35,000/yr | R80,000 | n/a (not a coverage uplift) | enables source-removal hand-off | n/a | 4–8 wks | NDoH 2023 CIF guideline |
| S3: Sentinel toxicology (agent attribution — distinct purpose, research-funded) | — (incl. in yr 1) | `r sprintf("R%.1fM/yr (5-site)", round(as.numeric(pick("c_option6_full_5site_annual"))/1e6,1))` | `r sprintf("R%.1fM (1-site pilot)", round(as.numeric(pick("c_option6_pilot_year1"))/1e6,1))` | n/a (not a coverage uplift) | feeds product-banning evidence base | n/a | 6–12 mo pilot | — |

: **Intermediate-outcome costing analysis — surveillance options, base-case.** Costs are 2024–25 ZAR; P_detect is the per-case detection probability anchored to the StatsSA MACOD 2023 mid-burden of `r scales::comma(as.numeric(pick("n_true_burden_from_statssa_mid")))` true cases/year. †**R / pp** = annual operating cost ÷ percentage-point gain in P_detect over status quo (e.g. R9,716 / pp means each additional 1% of cases detected costs R9,716/yr to sustain). S1 and S2 expand coverage within the formal healthcare system; **S3 is a parallel agent-attribution investment**, not a coverage uplift — see option text above. **S4 is a data-quality / investigation-form upgrade** that converts detected cases into actionable source-removal hand-offs to the [Coordination brief](coordination_policy.qmd); it does not raise P_detect by design. **S1b is a Phase 2 community layer**, commissioned conditional on MVP pilot data confirming a structural rural/informal gap (trigger: ≥40% of severe cases in low-facility-access districts after 6 months of MVP operation) **and** on PIH/NMC opening a non-clinician intake channel — without that pre-condition the CHW network has no destination for its reports and S1b yields no P_detect gain. {.striped .hover}
