...
| Option | Setup (one-off) | Annual operating (yr 2+) | Year 1 total | P_detect (per-case) | Δ vs. status quo | Time | Parameter |
|:-------|----------------:|-------------------------:|-------------:|:-------------------:|:----------------:|:-----|:----------|
| Status quo | — | R0 | R0 | $\frac{1,013}{20,960} = 0.048$ | — | — | [`d_status_quo`](../parameters_hub.qmd#tbl-detection) |
| S1: PIH dashboard + MVD (+AfriTox agent name) | `r zar(as.numeric(pick("c_S1_PIH_MVD_setup")))` | `r zar(as.numeric(pick("c_S1_PIH_MVD_op")))` | `r zar(as.numeric(pick("c_S1_PIH_MVD_setup")) + as.numeric(pick("c_S1_PIH_MVD_op")))` | $\frac{1,013 + 1,158}{20,960} = 0.104$ | `r sprintf("+%.1f pp", (as.numeric(pick("d_S1_PIH_MVD")) - as.numeric(pick("d_status_quo"))) * 100)` | 1–2 wks dash / 3–9 mo MVD | [`d_S1_PIH_MVD`](../parameters_hub.qmd#tbl-detection) |
| S2: BChE auto-notify | `r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")))` | `r zar(as.numeric(pick("c_S2_BChE_autonotify_op")))` | `r zar(as.numeric(pick("c_S2_BChE_autonotify_setup")) + as.numeric(pick("c_S2_BChE_autonotify_op")))` | $\frac{1,013 + 1,779}{20,960} = 0.133$ | `r sprintf("+%.1f pp", (as.numeric(pick("d_S2_BChE_autonotify")) - as.numeric(pick("d_status_quo"))) * 100)` | 2–4 wks | [`d_S2_BChE_autonotify`](../parameters_hub.qmd#tbl-detection) |
| **S1 + S2 (Surveillance MVP)** | **`r zar(as.numeric(pick("c_surv_MVP_setup")))`** | **`r zar(as.numeric(pick("c_surv_MVP_op")))`** | **`r zar(as.numeric(pick("c_surv_MVP_setup")) + as.numeric(pick("c_surv_MVP_op")))`** | $\frac{1,013 + 1,779 + 1,158}{20,960} = 0.189$ | **`r sprintf("+%.1f pp", (as.numeric(pick("d_surveillance_mvp")) - as.numeric(pick("d_status_quo"))) * 100)`** | **12 weeks** | [`d_surveillance_mvp`](../parameters_hub.qmd#tbl-detection) |
| S1b: Phase 2 community layer *(pilot: FS + Gauteng; national conditional on pilot)* | `r sprintf("R%.0fM–R%.1fM", as.numeric(pick("c_S1b_setup_lo"))/1e6, as.numeric(pick("c_S1b_setup_hi"))/1e6)` | Pilot `r sprintf("R%.1fM/yr", as.numeric(pick("c_S1b_op_lo"))/1e6)`; national `r sprintf("R%.1fM/yr", as.numeric(pick("c_S1b_op_hi"))/1e6)` | `r sprintf("R%.1fM–R%.1fM", (as.numeric(pick("c_S1b_setup_lo")) + as.numeric(pick("c_S1b_op_lo")))/1e6, (as.numeric(pick("c_S1b_setup_hi")) + as.numeric(pick("c_S1b_op_hi")))/1e6)` | `r pick("d_S1b_event_community_lo")`–`r pick("d_S1b_event_community_hi")` (working: `r pick("d_S1b_event_community_mid")`) | `r sprintf("+%.0f to +%.0f pp", (as.numeric(pick("d_S1b_event_community_lo")) - as.numeric(pick("d_status_quo"))) * 100, (as.numeric(pick("d_S1b_event_community_hi")) - as.numeric(pick("d_status_quo"))) * 100)` | 12 mo pilot → national | [`d_S1b_event_community_mid`](../parameters_hub.qmd) |
| S3: Sentinel toxicology (agent attribution — distinct purpose) | — (incl. in yr 1) | `r sprintf("R%.1fM/yr (5-site)", round(as.numeric(pick("c_option6_full_5site_annual"))/1e6,1))` | `r sprintf("R%.1fM (1-site pilot)", round(as.numeric(pick("c_option6_pilot_year1"))/1e6,1))` | n/a (not a coverage uplift) | feeds product-banning evidence base | 6–12 mo pilot | — |

: Surveillance options — base-case. P_detect is the per-case detection probability, anchored to the StatsSA MACOD 2023 mid-burden of `r scales::comma(as.numeric(pick("n_true_burden_from_statssa_mid")))` true cases/year. S1 and S2 expand coverage within the formal healthcare system; **S3 is a parallel agent-attribution investment**, not a coverage uplift — see option text above. **S1b is a Phase 2 community layer**, commissioned conditional on MVP pilot data confirming a structural rural/informal gap (trigger: ≥40% of severe cases in low-facility-access districts after 6 months of MVP operation). Translating P_detect into cases or deaths averted requires the response-side parameters (η, agency capacity, regulatory action) covered in the [Coordination](coordination_policy.qmd) and [Terbufos](terbufos_policy.qmd) briefs. {.striped .hover}
...
**Setup.** `c_surv_MVP_setup` = R80,000 (one-off). The CSV note reads: *"S2 BChE HL7 work dominates."* This appears low relative to `c_S2_BChE_autonotify_setup` = R200,000 because `c_surv_MVP_setup` is the **incremental** setup cost when all three components are adopted together — the S0 NHLS-NMC feed governance and data-mapping infrastructure (already incurred, `c_S0_NHLS_NMC_feed_setup` = R80,000) is shared and not double-counted; S1 PIH setup (`c_S1_PIH_MVD_setup` = R40,000) is also separately parameterised. The R80,000 MVP setup row covers the marginal IT integration work above what S0 already established.
:::
...
::: {.callout-note appearance="minimal"}
**POPIA compliance.** All PIH-to-NMC transmissions must be de-identified before transmission: no name, phone number, or exact address. Age-band (not exact age), province/district (not street address), and anonymised TMS call ID are sufficient for surveillance linkage and comply with Section 90 of the National Health Act (2003) and POPIA. Exact age may be retained internally within PIH TMS for clinical purposes and transmitted as age-band only.
:::
